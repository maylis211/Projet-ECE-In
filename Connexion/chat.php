<?php
// Assumons que vous avez déjà inclus votre connexion PDO et démarré la session
require_once 'connexion_pdo.php'; // Votre script de connexion PDO
session_start();

// Récupération du nom de l'ami depuis l'URL
$friend_name = isset($_GET['friend']) ? urldecode($_GET['friend']) : null;

// Vérification si l'utilisateur est connecté et si le nom de l'ami est défini
if (isset($_SESSION['username']) && !empty($friend_name)) {
    $username = $_SESSION['username'];

    // Récupération des messages entre l'utilisateur et l'ami
    $stmt = $pdo->prepare("SELECT * FROM messages WHERE (username = ? AND friend_name = ?) OR (username = ? AND friend_name = ?) ORDER BY created_at ASC");
    $stmt->execute([$username, $friend_name, $friend_name, $username]);
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['message'])) {
        ...
        $stmt = $pdo->prepare("INSERT INTO messages (username, friend_name, content) VALUES (?, ?, ?)");

        // Exécution de la requête avec les valeurs des paramètres
        $stmt->execute([$username, $friend_name, $content]);

        // Redirection vers la même page pour afficher le nouveau message
        header("Location: chat.php?friend=" . urlencode($friend_name));
        exit;
    }
} else {
    $messages = [];
    ...
    <header>
        ...
        <?php if (!isset($_SESSION['username'])): ?>
            ...
            <?php else: ?>
                <div id="wrapper">
                    <div id="menu">
                        <p class="welcome">Conversation avec <b><?php echo htmlspecialchars($friend_name); ?></b></p>
                        <!-- Ajoutez le bouton de retour à la page d'accueil -->
                        <a href="home.php" id="back-to-home">Retour à l'accueil</a>
                    </div>
                    <div id="chatbox">
                        <?php foreach ($messages as $message): ?>
                            <div class="chat-message">
                                <?php $isUserMessage = ($message['username'] === $username); ?>
